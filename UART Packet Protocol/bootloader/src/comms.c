...
#define COMMS_RECV_PACKET_BUFFER_SIZE (4U)
...
void comms_setup(void) {
    // Retx Packet
    retx_packet.length = 1U;
    retx_packet.data[0] = COMMS_RETX_PACKET_DATA0;

    // Ack Packet
    ack_packet.length = 1U;
    ack_packet.data[0] = COMMS_ACK_PACKET_DATA0;
    ...
    // CRC Calculation
    retx_packet.crc = crc8((uint8_t *) &retx_packet, COMMS_PACKET_FULL_LEN - COMMS_PACKET_CRC_LEN);
    ack_packet.crc = crc8((uint8_t *) &ack_packet, COMMS_PACKET_FULL_LEN - COMMS_PACKET_CRC_LEN);
}

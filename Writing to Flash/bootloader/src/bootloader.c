...
int main(void) {
    ...
    uint8_t data[1024] = {0U};
    for (uint32_t i=0; i<1024; i++) {
        data[i] = i&0xFF;
    }
    bl_flash_erase_main_application();
    bl_flash_write(APP_START_ADDRESS, data, 1024);
    bl_flash_write(APP_START_ADDRESS + 1024, data, 1024);
    bl_flash_write(APP_START_ADDRESS + 2048, data, 1024);
